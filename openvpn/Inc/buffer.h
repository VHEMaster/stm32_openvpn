 ...
 */
void buf_write_string_file(const struct buffer *buf, const char *filename, FIL* fd);
...
#define ALLOC_OBJ(dptr, type) \
    { \
        check_malloc_return((dptr) = (type *) openvpn_malloc(sizeof(type))); \
    }
...
#define ALLOC_ARRAY(dptr, type, n) \
    { \
        check_malloc_return((dptr) = (type *) openvpn_malloc(array_mult_safe(sizeof(type), (n), 0))); \
    }
