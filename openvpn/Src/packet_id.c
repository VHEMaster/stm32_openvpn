...
packet_id_persist_load(struct packet_id_persist *p, const char *filename)
{
    ...
    if (!packet_id_persist_enabled(p))
    {
        ...
        if (f_open(&p->fd, filename, FA_CREATE_ALWAYS | FA_WRITE | FA_READ) != FR_OK)
        {
        ...
        else
        {
            ...
            p->filename = filename;
            if (f_read(&p->fd, &image, sizeof(image), &n) == FR_OK && n == sizeof(image))
            {
...
packet_id_persist_save(struct packet_id_persist *p)
{
                                                    ...
                                                    || p->id != p->id_last_written))
    {
        ...
        image.time = p->time;
        image.id = p->id;
        seek_ret = f_lseek(&p->fd, (off_t)0);
        if (seek_ret == (off_t)0)
        {
            if (f_write(&p->fd, &image, sizeof(image), &n) == FR_OK && n == sizeof(image))
            {
