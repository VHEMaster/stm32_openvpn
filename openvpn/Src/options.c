...
usage(void)
{
    ...
    printf("Usage message not available\n");
...
in_src_get(const struct in_src *is, char *line, const int size)
{
    ...
    {
        return BOOL_CAST(f_gets(line, size, is->u.fp));
    }
...
static bool
check_inline_file_via_fp(FIL *fp, char *p[], struct gc_arena *gc)
{
    struct in_src is;
    is.type = IS_TYPE_FP;
    is.u.fp = fp;
