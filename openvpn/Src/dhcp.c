...
static int
get_dhcp_message_type(const struct dhcp_openvpn *dhcp, const int optlen)
{
...
static in_addr_t
do_extract(struct dhcp_openvpn *dhcp, int optlen)
{
...
{
    struct dhcp_openvpn_full *df = (struct dhcp_openvpn_full *) BPTR(ipbuf);
    const int optlen = BLEN(ipbuf) - (sizeof(struct openvpn_iphdr) + sizeof(struct openvpn_udphdr) + sizeof(struct dhcp_openvpn));
        ...
        && df->dhcp.op == BOOTREPLY)
    {
        ...
        if (message_type == DHCPACK || message_type == DHCPOFFER)
        {
            ...
            df->udp.check = htons(udp_checksum((uint8_t *) &df->udp,
                                               sizeof(struct openvpn_udphdr) + sizeof(struct dhcp_openvpn) + optlen,
                                               (uint8_t *)&df->ip.saddr,
                                               (uint8_t *)&df->ip.daddr));
