...
verify_callback(int preverify_ok, X509_STORE_CTX *ctx)
{
    ...
    if (R_SUCCESS != verify_cert(session, current_cert, X509_STORE_CTX_get_error_depth(ctx)))
    {
        goto cleanup;
    }
                       ...
                       int size)
{
    ...
    *out = '\0';
    ...
    {
        const result_t ret = (strlen((char *)buf) < size) ? R_SUCCESS : R_FAILURE;
        OPENSSL_free(buf);
        return ret;
    }
}
...
#endif
    if (R_FAILURE == extract_x509_field_ssl(X509_get_subject_name(peer_cert),
...
do_setenv_x509(struct env_set *es, const char *name, char *value, int depth)
{
    ...
    name_expand = (char *) openvpn_malloc(name_expand_size);
    check_malloc_return(name_expand);
    openvpn_snprintf(name_expand, name_expand_size, "X509_%d_%s", depth, name);
    setenv_str(es, name_expand, value);
    openvpn_free(name_expand);
}
...
x509_verify_ns_cert_type(openvpn_x509_cert_t *peer_cert, const int usage)
{
    ...
    if (usage == NS_CERT_CHECK_CLIENT)
    {
        ...
        if (result == R_FAILURE)
        {
            ...
            result = (ns && ns->length > 0 && (ns->data[0] & NS_SSL_CLIENT)) ? R_SUCCESS : R_FAILURE;
            if (result == R_SUCCESS)
            {
    ...
    if (usage == NS_CERT_CHECK_SERVER)
    {
        ...
        if (result == R_FAILURE)
        {
            ASN1_BIT_STRING *ns;
            ns = X509_get_ext_d2i(peer_cert, NID_netscape_cert_type, NULL, NULL);
            result = (ns && ns->length > 0 && (ns->data[0] & NS_SSL_SERVER)) ? R_SUCCESS : R_FAILURE;
            if (result == R_SUCCESS)
            {
                    ...
                    int expected_len)
{
    ...
    result_t fFound = R_FAILURE;
    for (size_t i = 0; fFound != R_SUCCESS && i < expected_len; i++)
    {
...
x509_verify_cert_eku(X509 *x509, const char *const expected_oid)
{
    ...
    else
    {
        ...
        for (i = 0; R_SUCCESS != fFound && i < sk_ASN1_OBJECT_num(eku); i++)
        {
            ASN1_OBJECT *oid = sk_ASN1_OBJECT_value(eku, i);
            char szOid[1024];

            if (R_SUCCESS != fFound && OBJ_obj2txt(szOid, sizeof(szOid), oid, 0) != -1)
            {
            ...
            if (R_SUCCESS != fFound && OBJ_obj2txt(szOid, sizeof(szOid), oid, 1) != -1)
            {
