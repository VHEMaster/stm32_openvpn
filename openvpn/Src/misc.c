    ...
    for (current = *list, prev = NULL; current != NULL; current = current->next)
    {
        if (env_string_equal(current->string, str))
        {
            ...
            if (do_free)
            {
                ...
                openvpn_free(current->string);
                openvpn_free(current);
            }
            return true;
        }
        prev = current;
...
create_temp_file(const char *directory, const char *prefix, struct gc_arena *gc)
{
    ...
    do
    {
         ...
         * exists.  */
				FRESULT res = f_open(&fd, retfname, FA_CREATE_NEW);
        if (res == FR_OK)
        {
            f_close(&fd);
            return retfname;
                 ...
                 const char *auth_challenge)
{
    ...
    if (!up->defined)
    {
        ...
        else if (from_authfile && !(flags & GET_USER_PASS_DYNAMIC_CHALLENGE))
        {
            ...
            if (f_open(&fp, auth_file, FA_READ) != FR_OK)
            {
            ...
            {
                /* Read username first */
                if (f_gets(up->username, USER_PASS_LEN, &fp) == NULL)
                {
            ...
            if (f_gets(password_buf, USER_PASS_LEN, &fp) != NULL)
            {
                chomp(password_buf);
            }
