...
set_common_name(struct tls_session *session, const char *common_name)
{
    ...
    {
        openvpn_free(session->common_name);
                 ...
                 const char *subject, const char *common_name)
{
    ...
    {
        if (R_SUCCESS == x509_verify_ns_cert_type(peer_cert, opt->ns_cert_type))
        {
        ...
        else
        {
            ...
            return R_FAILURE;             /* Reject connection */
        }
    }
    ...
    {
        if (R_SUCCESS == x509_verify_cert_ku(peer_cert, opt->remote_cert_ku, MAX_PARMS))
        {
    ...
    {
        if (R_SUCCESS == x509_verify_cert_eku(peer_cert, opt->remote_cert_eku))
        {
...
verify_cert_export_cert(openvpn_x509_cert_t *peercert, const char *tmp_dir, struct gc_arena *gc)
{
    ...
    if (f_open(&percert_file, peercert_filename, FA_CREATE_ALWAYS) != FR_OK)
    {
        msg(M_ERR, "Failed to open temporary file : %s", peercert_filename);
        return NULL;
    }

    if (R_SUCCESS != x509_write_pem(&percert_file, peercert))
    {
        msg(M_ERR, "Error writing PEM file containing certificate");
    }

    f_close(&percert_file);
...
verify_cert(struct tls_session *session, openvpn_x509_cert_t *cert, int cert_depth)
{
    ...
    if (R_SUCCESS != backend_x509_get_username(common_name, sizeof(common_name),
    ...
    if (cert_depth == 0 && R_SUCCESS != verify_peer_cert(opt, cert, subject, common_name))
    {
        goto cleanup;
    }

    /* call --tls-verify plug-in(s), if registered */
    if (R_SUCCESS != verify_cert_call_plugin(opt->plugins, opt->es, cert_depth, cert, subject))
    {
        goto cleanup;
    }

    /* run --tls-verify script */
    if (opt->verify_command && R_SUCCESS != verify_cert_call_command(opt->verify_command,
    ...
    if (opt->crl_file)
    {
        ...
        {
            if (R_SUCCESS != verify_check_crl_dir(opt->crl_file, cert))
            {
                goto cleanup;
            }
        }
...
man_def_auth_set_client_reason(struct tls_multi *multi, const char *client_reason)
{
    ...
    {
        openvpn_free(multi->client_reason);
...
key_state_rm_auth_control_file(struct key_state *ks)
{
    if (ks && ks->auth_control_file)
    {
        ...
        openvpn_free(ks->auth_control_file);
