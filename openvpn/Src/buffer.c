...
#else
    buf.data = openvpn_calloc(1, size);
#endif
...
#else
    ret.data = (uint8_t *) openvpn_malloc(buf->capacity);
#endif
...
void
buf_write_string_file(const struct buffer *buf, const char *filename, FIL* fd)
{
    const int len = strlen((char *) BPTR(buf));
		uint32_t size;
    f_write(fd, BPTR(buf), len, &size);
    if (size != len)
    {
...
#else
        e = (struct gc_entry *) openvpn_malloc(size + sizeof(struct gc_entry));
#endif
...
#else
    e = (struct gc_entry_special *) openvpn_malloc(sizeof(struct gc_entry_special));
#endif
    check_malloc_return(e);
...
char_class(const unsigned char c, const unsigned int flags)
{
    ...
    if ((flags & CC_ASCII) && (c <= 0x7F)) // isascii(c))
    {
        return true;
    }
...
buffer_list_file(const char *fn, int max_line_len)
{
    ...
    {
        char *line = (char *) openvpn_malloc(max_line_len);
        if (line)
        {
            bl = buffer_list_new(0);
            while (f_gets(line, max_line_len, &fp) != NULL)
            {
