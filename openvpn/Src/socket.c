                 ...
                 int msglevel)
{
    ...
    var_host = openvpn_malloc(strlen(hostname)+1);
		strcpy(var_host, hostname);
                    ...
                    struct addrinfo **res)
{
    ...
    if (status != 0) /* parse as numeric address failed? */
    {
        ...
        fmt = "RESOLVE: Cannot resolve host address: %s:%s (%d)";
        if ((flags & GETADDR_MENTION_RESOLVE_RETRY)
            && !resolve_retry_seconds)
        {
            fmt = "RESOLVE: Cannot resolve host address: %s:%s (%d) (I would have retried this name query if you had specified the --resolv-retry option.)";
        }
        ...
        if (!(flags & GETADDR_RESOLVE) || status == EAI_FAIL)
        {
            ...
            msg(msglevel, "RESOLVE: Cannot parse IP address: %s:%s (%d)",
                print_hostname,print_servname, status);
            goto done;
        }
                ...
                if (*signal_received) /* were we interrupted by a signal? */
                {
                    ...
                    else
                    {
                        ...
                        if (0 == status)
                        {
                            ...
                            freeaddrinfo(*res);
                            *res = NULL;
                            status = TRY_AGAIN; /* = temporary failure */
                            errno = EINTR;
                        }
                        goto done;
                    }
    ...
    if (!sock->info.lsa->bind_local)
    {
        ...
        if (status !=0)
        {
            //msg(M_FATAL, "getaddrinfo() failed for local \"%s:%s\": %s",
                ...
                //gai_strerror(status));
            msg(M_FATAL, "getaddrinfo() failed for local \"%s:%s\": %d",
                sock->local_host, sock->local_port,
                status);
        }
    }
                  ...
                  struct gc_arena *gc)
{
    ...
    switch (sa->sa_family)
    {
        case AF_INET:
            ...
            addr_is_defined = ((struct sockaddr_in *) sa)->sin_addr.s_addr != 0;
						ip4addr_ntoa_r((const ip4_addr_t *)&((struct sockaddr_in *) sa)->sin_addr,hostaddr,sizeof(hostaddr));
						snprintf(servname, sizeof(servname), "%d", ((struct sockaddr_in *) sa)->sin_port);
            break;
        ...
        case AF_INET6:
            ...
            addr_is_defined = !IN6_IS_ADDR_UNSPECIFIED(&((struct sockaddr_in6 *) sa)->sin6_addr);
						ip6addr_ntoa_r((const ip6_addr_t *)&((struct sockaddr_in6 *) sa)->sin6_addr,hostaddr,sizeof(hostaddr));
						snprintf(servname, sizeof(servname), "%d", ((struct sockaddr_in6 *) sa)->sin6_port);
            break;
		...
		//Bypass... I don't really know how to do it.
    ...
    if (status!=0)
    {
        ...
        buf_printf(&out,"[nameinfo() err: %d]",status);
        return BSTR(&out);
    }
...
#error ENABLE_IP_PKTINFO is set without IP_PKTINFO xor IP_RECVDSTADDR (fix syshead.h)
                ...
                case AF_INET6:
                {
                    ...
                    //                buf, sizeof(buf), NULL, 0, NI_NUMERICHOST) == 0)
                    {
												...
												ip6addr_ntoa_r((const ip6_addr_t *)&sin6.sin6_addr,buf,sizeof(buf));
            ...
            else
            {
                ...
                //getnameinfo(&addr->addr.sa, sizeof(struct sockaddr_in6),
                            //buf, sizeof(buf), NULL, 0, NI_NUMERICHOST);
								ip6addr_ntoa_r((const ip6_addr_t *)&addr->addr.in6.sin6_addr,name_buf,sizeof(name_buf));	
