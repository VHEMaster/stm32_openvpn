...
show_available_ciphers(void)
{
    ...
    while (*ciphers != 0)
    {
        const cipher_kt_t *info = mbedtls_cipher_info_from_type((const mbedtls_cipher_type_t)*ciphers);
...
show_available_digests(void)
{
    ...
    while (*digests != 0)
    {
        const mbedtls_md_info_t *info = mbedtls_md_info_from_type((const mbedtls_md_type_t)*digests);
