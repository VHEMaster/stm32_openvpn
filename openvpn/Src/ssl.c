...
{
		//Here was deleted a lot of code 
...
init_ssl(const struct options *options, struct tls_root_ctx *new_ctx)
{
             ...
             && (options->cert_file || options->management_flags & MF_EXTERNAL_CERT))
    {
        ...
        else
        {
            ...
            openvpn_free(external_certificate);
        }
    }
#endif
    else
...
key_state_free(struct key_state *ks, bool clear)
{
    ...
    if (ks->send_reliable)
    {
        ...
        openvpn_free(ks->send_reliable);
    }

    if (ks->rec_reliable)
    {
        reliable_free(ks->rec_reliable);
        openvpn_free(ks->rec_reliable);
    }

    if (ks->rec_ack)
    {
        openvpn_free(ks->rec_ack);
    }

    if (ks->key_src)
    {
        openvpn_free(ks->key_src);
    }
...
tls_multi_free(struct tls_multi *multi, bool clear)
{
...
#endif
#if P2MP_SERVER
    openvpn_free(multi->peer_info);
#endif

    if (multi->locked_cn)
    {
        openvpn_free(multi->locked_cn);
    }

    if (multi->locked_username)
    {
        openvpn_free(multi->locked_username);
    }
    ...
    openvpn_free(multi->remote_ciphername);
