...
tls_ctx_free(struct tls_root_ctx *ctx)
{
    if (ctx)
    {
        ...
        if (ctx->priv_key_pkcs11 != NULL)
        {
            ...
            openvpn_free(ctx->priv_key_pkcs11);
        }
#endif
#if defined(MANAGMENT_EXTERNAL_KEY)
        if (ctx->external_key != NULL)
        {
            openvpn_free(ctx->external_key);
        }
#endif

        if (ctx->allowed_ciphers)
        {
            openvpn_free(ctx->allowed_ciphers);
        }
...
{
    buffer_entry *new_block = openvpn_malloc(sizeof(buffer_entry));
    if (NULL == new_block)
    {
        return MBEDTLS_ERR_NET_SEND_FAILED;
    }

    new_block->data = openvpn_malloc(len);
    if (NULL == new_block->data)
    {
        openvpn_free(new_block);
        return MBEDTLS_ERR_NET_SEND_FAILED;
    }
