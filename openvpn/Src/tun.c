...
{
	//WE ARE CURRENTLY INIT open_tun_generic IT HERE
    char tunname[256];
...
close_tun_generic(struct tuntap *tt)
{
    ...
    {
				//This actually used
				openvpnif_dispose(tt->fd);
        //close(tt->fd);
    }
    if (tt->actual_name)
    {
        openvpn_free(tt->actual_name);
    }
    clear_tuntap(tt);
}
#endif /* !_WIN32 */
...
open_tun(const char *dev, const char *dev_type, const char *dev_node, struct tuntap *tt)
{
    ...
    tt->actual_name = (char *) openvpn_malloc(32);
...
write_tun(struct tuntap *tt, uint8_t *buf, int len)
{
		//This actually used
    //return write(tt->fd, buf, len);
    return openvpnif_write(tt->fd, buf, len);
}
...
read_tun(struct tuntap *tt, uint8_t *buf, int len)
{
		//This actually used
    //return read(tt->fd, buf, len);
    return openvpnif_read(tt->fd, buf, len);
}
