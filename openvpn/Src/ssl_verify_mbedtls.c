                ...
                uint32_t *flags)
{
    ...
    else if (R_SUCCESS != verify_cert(session, cert, cert_depth))
    {
                    ...
                    int expected_len)
{
    ...
    for (size_t i = 0; R_SUCCESS != fFound && i<expected_len; i++)
    {
        if (expected_ku[i] != 0
...
result_t
x509_write_pem(FIL *peercert_file, mbedtls_x509_crt *peercert)
{
