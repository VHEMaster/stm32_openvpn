    ...
    )
{
    ...
    *base64 = NULL;
    ...
    if ((internal_id = (char *)openvpn_malloc(max)) == NULL)
    {
    ...
    if ((certificate_blob = (unsigned char *)openvpn_malloc(certificate_blob_size)) == NULL)
    {
        msg(M_FATAL, "PKCS#11: Cannot allocate memory");
        goto cleanup;
    }
...
cleanup:
    ...
    if (internal_id != NULL)
    {
        openvpn_free(internal_id);
        internal_id = NULL;
    }

    if (internal_base64 != NULL)
    {
        openvpn_free(internal_base64);
        internal_base64 = NULL;
    }

    if (certificate_blob != NULL)
    {
        openvpn_free(certificate_blob);
    ...
    )
{
    ...
    for (current = user_certificates; current != NULL; current = current->next)
    {
        ...
        if (
            rv == CKR_OK
            && (ser = (char *)openvpn_malloc(ser_len)) == NULL
            )
        {
            msg(M_FATAL, "PKCS#11: Cannot allocate memory");
            goto cleanup1;
